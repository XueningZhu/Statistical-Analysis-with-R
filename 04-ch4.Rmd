...
```{r ch4-plot1, warning=FALSE, eval=F, echo = T}
# 统计小说类型频数
a=table(novel$小说类型)
# 对频数排序
...
```{r ch4-plot2, warning=FALSE, eval=F}
# 求得各类型小说平均点击数的均值
means<-novel%>%
  group_by(小说类型)%>%
...
```{r ch4-plot3, warning=FALSE, eval = F}
# 构建绘图所需数据矩阵d
...
```{r ch4-plot6, warning=FALSE, eval = F}
# 绘制直方图
par(mfrow=c(1,2),family = "STXihei",las=2)
...
```{r ch4-plot18, warning=FALSE, eval = F}
...
```{r ch4-plot20, warning=FALSE, eval = F}
ggplot(bidf_plot)+
...
```{r ch4-plot21, warning=FALSE, eval = F}
ggplot(fin_data)+
...
```{r ch4-plot22, warning=FALSE, eval = F}
###绘制并列柱状图
...
```{r ch4-plot23, warning=FALSE, eval = F}
#绘制箱线图
ggplot(fin_data)+
