...
# 绘制柱状图
par(family = "STHeiti",las = 2)
barplot(a[1:5],names.arg = names(a)[1:5],family="STHeiti",ylab = "频数",xlab="",col = c("gold","grey","grey","grey","grey"))
title("各类型小说频数分布柱状图",family="STHeiti")
```
...
par(family = "STHeiti",las = 2)
# 绘制均值柱状图
barplot(means$mean[1:5], names.arg=means$小说类型[1:5],family="STHeiti",ylab = "总点击数(万次)",xlab="",col = c("gold","grey","grey","grey","grey"))
title("各类型小说平均总点击数",family="STHeiti")
...
# 绘制分组柱状图(beside = T)
par(mfrow=c(1,2), family = "STHeiti",las=2)
...
# 绘制柱状图
p = p + geom_bar(width = 1,stat = "identity")+theme_gray(base_family = 'STHeiti') + scale_fill_brewer('',palette = "Blues")
```
...
# 绘制直方图
p + geom_histogram(bins = 30) + theme_gray(base_family = 'STHeiti') + xlab('对数总点击数') + ylab('频数')
```
...
# 绘制分组箱线图
p+ geom_boxplot(fill = c('gold','grey')) + theme_gray(base_family = 'STHeiti') + xlab('小说性质') + ylab('对数总点击数')
```
...
# 绘制散点图
p + geom_point() + theme_gray(base_family = 'STHeiti') + xlab('总点击数') + ylab('评论数')
```
...
ggplot(travel_dat)+
  ...
  xlab("产品价格（对数变换）") +
  ylab("频数") +
  theme_bw(base_family = "STHeiti")+                         #设置背景颜色和字体
...
```{r ch4-plot19}
par(las = 2,mai = c(1,1,0.5,1),family = "STHeiti")
...
ggplot(fin_data_city)+
  ...
  theme_bw(base_family = "STHeiti")+#背景设置为白色，字体为宋体
  labs(fill = "岗位类别")+#设置图例
